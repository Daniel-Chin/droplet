...
%We now take a closer look at the behaviors governed by equation \ref{eq:friction}. 
...
\begin{itemize}
    ...
    \item The resulting macroscopic behaviors of extended GNBC are analogous to how rigid bodies display static and sliding frictions. If $F_\text{static limit}$ is bounding, we have sliding friction (slip), otherwise we have static friction (no-slip). 
\end{itemize}
...
\end{figure}
In figure ~\ref{fig:split}, a droplet separates into two smaller ones. After the splicing moment, surface tension quickly brings the sharp edge into the body of the droplet. In figure ~\ref{fig:wall_merge}, two droplets coalesce on the wall. This is one of the six basic cases of interface splicing. 
