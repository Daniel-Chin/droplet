...
% does not work as well as the point method. 

% first spread, then fill!!!
gravity = zeros(N, N);

gravity_helper = ones(Nb, 2) .* [1 0];
gravity_soul = round([L / 4, center_mass] / h);

surface_grid = vec_spread(gravity_helper, X, dtheta, Nb);
  ...
  for gravity_frontier_i = 1 : gravity_frontier_top
    ...
    if gravity_x ~= 1
      if surface_grid(gravity_x, gravity_y, 1) >= job(3) * 1.1
        ...
        gravity(gravity_x, gravity_y) = 1-job_3;
...
center_mass = mean(rmmissing((gravity * [0 : h : L-h]') ./ sum(gravity, 2)));
